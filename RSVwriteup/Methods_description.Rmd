...
##########################################################################################
```
In this case, the total number of excess dearhs in people 65 and older due to influenza over six seasons is estimated at $`r round(EM,digits=2)`$. Confidence intervals for this estimate can be obtained using bootstrapping or implementing the model in a Bayesian framework (using flat priors).
...
Even though the results from these ecological analyses are guesses, which rely on certain assumptions, such high numbers of RSV excess mortality are quite implausible. I believe that this is a result of the strong  colinearilty between the RSV indicator and RC mortality (correlation coefficient , 
p-value $< 0.00001$)
\clearpage
```{r fig1,eval=TRUE,fig.pos='ht',echo=FALSE,fig.width=8,fig.height=6}
